...
1. **accesible: resuelto por criterio de practicabilidad (2026-07-17) + correccion de Sector 1 (2026-07-21).** El canonico adopto un criterio derivado de `ancho_m` (>= 0.90 m = practicable; ver seccion "Criterio de practicabilidad" mas abajo). Bajo ese criterio, los 5 pasillos internos (S1 1.67 m, S3 principal 1.50 m, S3 lateral 1.47 m, S4 principal 1.58 m, S4 lockers 1.08 m) dan `accesible=true` - coinciden con el seed v5 en los 5 casos.
   - **Divergencia `s1_bano_chico` (id v5 6) RESUELTA (2026-07-21):** medido 1.62 x 1.88 m. En v5 figuraba `accesible=true`; en el canonico originalmente era `false` por espacio de maniobra interior insuficiente (circulo 1,50 m). Re-evaluado: el radio de maniobra es suficiente. Ambos modelos coinciden ahora en `accesible=true`. Ya no quedan divergencias de `accesible` entre v5 y el canonico.
...
**Ancho de paso vs espacio de maniobra interior:** son dos cosas distintas. El ancho de paso es el ancho libre de un tramo/corredor por el que se circula; el espacio de maniobra interior es el area libre dentro de un recinto (tipicamente un bano) necesaria para girar una silla de ruedas (circulo de 1,50 m de diametro, Art. 4.8.2.5 Ley 962). Un tramo puede ser practicable por ancho de paso sin que el recinto al que conduce tenga espacio de maniobra suficiente - `s1_bano_chico` fue evaluado bajo este criterio (ver b.1) y en la re-evaluacion del 2026-07-21 se confirmo que si tiene radio de maniobra suficiente.
